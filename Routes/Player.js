...
router.post('/', [
    ...
    try {
        const Player = await User.findById(req.user.id).select('-password').lean();
        const Ids = await User.findById(req.user.id).select('-password -email -name -date').lean();
        const _id = Ids
...
router.put('/:id', auth, async (req, res) => {
    const pl = await Info.findById(req.params.id).lean();
